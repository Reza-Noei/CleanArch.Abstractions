...
### 1. Register Mediator and your Application Assemblies
```
...
### 2. Enable ASP.NET Core integration

```Charp
builder.Services.AddMediator();
```

### 3. Map endpoints automatically
...
### Command
```Csharp
[HttpEndpoint(Method = HttpMethod.Post, Route = "users")]
...
{
    opt.AddApplicationAssembly(typeof(Program).Assembly);
    opt.AddBehavior(typeof(LoggingBehavior<,>));
});

builder.Services.AddMediatorAspNetCore();
